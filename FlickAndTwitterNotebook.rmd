...
```{r}
hatchFlickr5 <- GetPhotos(query = "Hatch Lake", startDate = "6/29/2017")
dim(hatchFlickr5) ##gets 500 flickr photos
hatchFlickr5$DateTaken <- sapply(hatchFlickr5$id, GetDate) ##this takes awhile
hatchFlickr5 ## this is it for hatch lake


hatch1.1.2013_7.31.2013 
hatch8.1.2013_5.9.2014
hatch5.10.2014_5.7.2015
hatch5.8.2015_4.25.2016
hatch4.26.2016_6.29.2017
hatch6.29.2017_hatch6.21.2018

hatchFlickr5 <- rbind(hatch1.1.2013_7.31.2013, hatch8.1.2013_5.9.2014, hatch5.10.2014_5.7.2015, hatch5.8.2015_4.25.2016, hatch4.26.2016_6.29.2017, hatch6.29.2017_hatch6.21.2018)

hatchFlickr5$Year <- substr(hatchFlickr5$DateTaken, 1, 4)
hatchFlickr5$Month <- substr(hatchFlickr5$DateTaken, 6, 7)
hatchFlickr5$Day <- substr(hatchFlickr5$DateTaken, 9, 10)

hatchUnique <- hatchFlickr5%>%group_by(owner, Year, Month, Day)%>%summarise(Count = n())
dim(hatchUnique) ##1382 unique days

hatchVPM <- hatchUnique%>%group_by(Year, Month)%>%summarise(Count = n())
hatchVPM$Group = 1
hatchVPM$EasyDay <- paste(hatchVPM$Year, hatchVPM$Month, sep = "-")

hatchVPY <- hatchUnique%>%group_by(Year)%>%summarise(Count = n())
hatchVPY$Group = 1

ggplot(data = hatchVPM, aes(x = EasyDay, y = Count, group = Group)) + geom_line() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
ggplot(data = hatchVPY, aes(x = Year, y = Count, group = Group)) + geom_line()
```
...
```{r}
squashFlickr5 <- GetPhotos(query = "Squash Lake", startDate = "1/1/2013")
dim(squashFlickr5) ##gets 500 flickr photos
squashFlickr5$DateTaken <- sapply(squashFlickr5$id, GetDate) ##this takes awhile
squashFlickr5 ## this is it for squash lake

squashFlickr5$Year <- substr(squashFlickr5$DateTaken, 1, 4)
squashFlickr5$Month <- substr(squashFlickr5$DateTaken, 6, 7)
squashFlickr5$Day <- substr(squashFlickr5$DateTaken, 9, 10)

squashUnique <- squashFlickr5%>%group_by(owner, Year, Month, Day)%>%summarise(Count = n())
dim(squashUnique) ## 144 unique days

squashVPM <- squashUnique%>%group_by(Year, Month)%>%summarise(Count = n())
squashVPM$Group = 1
squashVPM$EasyDay <- paste(squashVPM$Year, squashVPM$Month, sep = "-")

squashVPY <- squashUnique%>%group_by(Year)%>%summarise(Count = n())
squashVPY$Group = 1

ggplot(data = squashVPM, aes(x = EasyDay, y = Count, group = Group)) + geom_line() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
ggplot(data = squashVPY, aes(x = Year, y = Count, group = Group)) + geom_line()
```
...
```{r}
mcgeeFlickr5 <- GetPhotos(query = "McGee Lake", startDate = "1/1/2013")
dim(mcgeeFlickr5) ##gets 500 flickr photos
mcgeeFlickr5$DateTaken <- sapply(mcgeeFlickr5$id, GetDate) ##this takes awhile
mcgeeFlickr5 ## this is it for mcgee lake

mcgeeFlickr5$Year <- substr(mcgeeFlickr5$DateTaken, 1, 4)
mcgeeFlickr5$Month <- substr(mcgeeFlickr5$DateTaken, 6, 7)
mcgeeFlickr5$Day <- substr(mcgeeFlickr5$DateTaken, 9, 10)

mcgeeUnique <- mcgeeFlickr5%>%group_by(owner, Year, Month, Day)%>%summarise(Count = n())
dim(mcgeeUnique) ## 48 unique days

mcgeeVPM <- mcgeeUnique%>%group_by(Year, Month)%>%summarise(Count = n())
mcgeeVPM$Group = 1
mcgeeVPM$EasyDay <- paste(mcgeeVPM$Year, mcgeeVPM$Month, sep = "-")

mcgeeVPY <- mcgeeUnique%>%group_by(Year)%>%summarise(Count = n())
mcgeeVPY$Group = 1

ggplot(data = mcgeeVPM, aes(x = EasyDay, y = Count, group = Group)) + geom_line() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
ggplot(data = mcgeeVPY, aes(x = Year, y = Count, group = Group)) + geom_line()
```
...
```{r}
towandaFlickr5 <- GetPhotos(query = "Towanda Lake", startDate = "1/1/2013")
dim(towandaFlickr5) ##gets 500 flickr photos
towandaFlickr5$DateTaken <- sapply(towandaFlickr5$id, GetDate) ##this takes awhile
towandaFlickr5 ## this is it for towanda lake

towandaFlickr5$Year <- substr(towandaFlickr5$DateTaken, 1, 4)
towandaFlickr5$Month <- substr(towandaFlickr5$DateTaken, 6, 7)
towandaFlickr5$Day <- substr(towandaFlickr5$DateTaken, 9, 10)

towandaUnique <- towandaFlickr5%>%group_by(owner, Year, Month, Day)%>%summarise(Count = n())
dim(towandaUnique) ## 9 unique days

towandaVPM <- towandaUnique%>%group_by(Year, Month)%>%summarise(Count = n())
towandaVPM$Group = 1
towandaVPM$EasyDay <- paste(towandaVPM$Year, towandaVPM$Month, sep = "-")

towandaVPY <- towandaUnique%>%group_by(Year)%>%summarise(Count = n())
towandaVPY$Group = 1

ggplot(data = towandaVPM, aes(x = EasyDay, y = Count, group = Group)) + geom_line() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
ggplot(data = towandaVPY, aes(x = Year, y = Count, group = Group)) + geom_line()
```
...
```{r}
taborFlickr5 <- GetPhotos(query = "Tabor Lake", startDate = "1/1/2013")
dim(taborFlickr5) ##gets 500 flickr photos
taborFlickr5$DateTaken <- sapply(taborFlickr5$id, GetDate) ##this takes awhile
taborFlickr5 ## this is it for tabor lake

taborFlickr5$Year <- substr(taborFlickr5$DateTaken, 1, 4)
taborFlickr5$Month <- substr(taborFlickr5$DateTaken, 6, 7)
taborFlickr5$Day <- substr(taborFlickr5$DateTaken, 9, 10)

taborUnique <- taborFlickr5%>%group_by(owner, Year, Month, Day)%>%summarise(Count = n())
dim(taborUnique) ## 172 unique days

taborVPM <- taborUnique%>%group_by(Year, Month)%>%summarise(Count = n())
taborVPM$Group = 1
taborVPM$EasyDay <- paste(taborVPM$Year, taborVPM$Month, sep = "-")

taborVPY <- taborUnique%>%group_by(Year)%>%summarise(Count = n())
taborVPY$Group = 1

ggplot(data = taborVPM, aes(x = EasyDay, y = Count, group = Group)) + geom_line() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
ggplot(data = taborVPY, aes(x = Year, y = Count, group = Group)) + geom_line()
```
...
```{r}
willowFlickr5 <- GetPhotos(query = "willow Flowage", startDate = "1/1/2013")
dim(willowFlickr5) ##gets 500 flickr photos
willowFlickr5$DateTaken <- sapply(willowFlickr5$id, GetDate) ##this takes awhile
willowFlickr5 ## this is it for willow lake

willowFlickr5$Year <- substr(willowFlickr5$DateTaken, 1, 4)
willowFlickr5$Month <- substr(willowFlickr5$DateTaken, 6, 7)
willowFlickr5$Day <- substr(willowFlickr5$DateTaken, 9, 10)

willowUnique <- willowFlickr5%>%group_by(owner, Year, Month, Day)%>%summarise(Count = n())
dim(willowUnique) ## 11 unique days

willowVPM <- willowUnique%>%group_by(Year, Month)%>%summarise(Count = n())
willowVPM$Group = 1
willowVPM$EasyDay <- paste(willowVPM$Year, willowVPM$Month, sep = "-")

willowVPY <- willowUnique%>%group_by(Year)%>%summarise(Count = n())
willowVPY$Group = 1

ggplot(data = willowVPM, aes(x = EasyDay, y = Count, group = Group)) + geom_line() + theme(axis.text.x = element_text(angle = 90, hjust = 1))
ggplot(data = willowVPY, aes(x = Year, y = Count, group = Group)) + geom_line()
```


Get summary stats for all of the lakes per year visitors
===

```{r}
mendotaVPY$Lake <- "LAKE MENDOTA"
onalaskaVPY$Lake <- "LAKE ONALASKA"
eaugalleVPY$Lake <- "LAKE EAU GALLE"
shawanoVPY$Lake <- "SHAWANO LAKE"
waubesaVPY$Lake <- "LAKE WAUBESA"
druidVPY$Lake <- "DRUID LAKE"
hatchVPY$Lake <- "HATCH LAKE"
willowVPY$Lake <- "WILLOW FLOWAGE"
squashVPY$Lake <- "SQUASH LAKE"
mcgeeVPY$Lake <- "MCGEE LAKE"
towandaVPY$Lake <- "TOWANDA LAKE"
taborVPY$Lake <- "TABOR LAKE"

ourLakes <- rbind(mendotaVPY, onalaskaVPY, eaugalleVPY, shawanoVPY, waubesaVPY, druidVPY, hatchVPY, willowVPY, squashVPY, mcgeeVPY, towandaVPY, taborVPY)

##only want to use full years of data so that means don't include 2018
ourLakes <- ourLakes %>% filter(Year != "2018")

##plot the lake data all together
ggplot(data = ourLakes, aes(x = Year, y = Count, group = Lake, col = Lake)) + geom_line()

ourLakesSummary <- ourLakes %>%
                    group_by(Lake) %>%
                    summarise(MeanVPY = mean(Count), MedianVPY = median(Count))

ourLakesSummary ##mean and median values are very similar, so could use either
plot(ourLakesSummary$MeanVPY, ourLakesSummary$MedianVPY);abline(a=0, b=1)##this plot shows that
```
