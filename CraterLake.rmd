...
ggplot(data = craterMonthlyVisitors, aes(y = log(RecVisit), x = log(Count))) +
  theme_bw() +
  ...
  ylab("Log Number of Recreational Visitors per Month") +
  ggtitle("Crater Lake Visitors per Month")

##This is like the Wood et al 2013 article
mod1 <- lm(log(Count) ~ log(RecVisit), data = craterMonthlyVisitors)
...
cor.test(log(craterMonthlyVisitors$RecVisit), log(craterMonthlyVisitors$Count)) ##.8477
```
