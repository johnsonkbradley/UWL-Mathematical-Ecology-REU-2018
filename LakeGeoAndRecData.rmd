...
```{r}
myLakes <- c("LAKE ONALASKA", "LAKE EAU GALLE", "WILLOW FLOWAGE", "TOWANDA LAKE", "TABOR LAKE", "SQUASH LAKE", "SHAWANO LAKE", "HATCH LAKE", "MCGEE LAKE", "LAKE MENDOTA", "LAKE WAUBESA", "DRUID LAKE", "LUDDEN LAKE", "RICE RIVER FLOWAGE", "HULLS LAKE", "BIG PORTAGE LAKE", "LAKE OWEN", "BIG EAU PLEINE RESEVOIR", "PARKER LAKE", "SHELL LAKE", "LAKE KOSHKONONG", "LAKE RIPLEY", "NAGAWICKA LAKE", "BIRCH LAKE")
myLakeDF <- wisComplete%>%filter(lagosname1 %in% myLakes)
##write.csv(myLakeDF2, "myLakesTry2.csv", row.names = F)


wisconsin <- get_map(location = "Wisconsin", zoom = 6)

ggmap(wisconsin) + 
  geom_point(data = myLakeDF, aes(x = nhd_long, y = nhd_lat, col = ClusterType, shape = EcoRegion), size = 5) + 
  scale_shape_manual(values = c(15, 16, 17, 18)) + 
  scale_color_manual(values = c("red", "blue", "green4")) + 
  scale_x_continuous(limits = c(-93, -87)) + 
  scale_y_continuous(limits = c(42, 47)) + 
  theme(legend.position = c(-89, 47))

##mapping the VPY for each lake, had to adjust for lake mendota's huge number
ggmap(wisconsin) +
  geom_point(data = myLakeDF[-10,], aes(x = nhd_long, y = nhd_lat, col = VPY)) +
  geom_point(data = myLakeDF[10,], aes(x = nhd_long, y = nhd_lat), col = "black") +
  scale_color_continuous(low = "cyan", high = "darkblue")
```


Merge myLakeDF with the vpyDF to combine the eco data with the social media data
===

```{r}
myLakeDF <- myLakeDF %>% left_join(vpyDF, by = c("lagosname1" = "LakeName"))
myLakeDF

myLakeDF$VPYRounded <- ceiling(myLakeDF$VPY)
```

Plots of VPY only 24 lakes
===

```{r}
ggplot(data = myLakeDF, aes(x = VPYRounded)) + geom_density(fill = "blue")
table(myLakeDF$VPYRounded)
```
...
```{r}
ggpairs(myLakeDF[,c(4,12,13,15,16,26,27,30,53,54)])
```
...
```{r}
##try logistic model with HadVisitors
logmod1 <- glm(HadVisitors ~ mean_TP + lake_area_ha + meandepth + pcturbancombined_hu12 + mean_secchi + Public.Beach + Public.Park, data = myLakeDF, family = "binomial")
summary(logmod1)

##try a linear model
linmod1 <- lm(VPY ~ mean_TP + lake_area_ha + meandepth + pcturbancombined_hu12 + mean_secchi + Public.Beach + Public.Park, data = myLakeDF)
summary(linmod1)

##try the poisson model
poismod1 <- glm(VPYRounded ~ mean_TP + lake_area_ha + meandepth + pcturbancombined_hu12 + mean_secchi + Public.Beach + Public.Park, data = myLakeDF, family = "poisson")
summary(poismod1)

##zip model  - this one is still weird: look into it more
zipmod1 <- zeroinfl(VPYRounded ~ mean_TP + lake_area_ha + meandepth + pcturbancombined_hu12 + mean_secchi + Public.Beach + Public.Park, data = myLakeDF)
summary(zipmod1)
```

Look at the largest lakes to sample from 
===

```{r}
wisComplete %>% select(lagosname1, lake_area_ha) %>% arrange(desc(lake_area_ha))

lakes60 <- read.csv("60lakes.csv", header = T)

myLakes3 <- wisComplete[wisComplete$lagosname1 %in% toupper(lakes60$Lake.Name),]
##write.csv(myLakes3, "myLakes3.csv", row.names = F)
##this is our 60 lake data set
ggmap(wisconsin) + 
  geom_point(data = wisComplete[wisComplete$lagosname1 %in% toupper(lakes60$Lake.Name),], aes(x = nhd_long, y = nhd_lat, col = ClusterType))
```

Setting up the data set for our 60 lakes
===

```{r}
lakesDF <- wisComplete %>% left_join(vpyDF, by = c("lagosname1" = "LakeName"))
lakesDF <- lakesDF[!is.na(lakesDF$VPY),]
lakesDF$VPYRounded <- ceiling(lakesDF$VPY)
```
...
```{r}
ggpairs(lakesDF[,c(4,12,13,15,16,26,27,30,53,54)]) ##with VPY: pcturban, lake_area; with Had Visitors: area, tp, pct ag
```
...
```{r}
##try logistic model with HadVisitors
logmod1a <- glm(HadVisitors ~ mean_TP + lake_area_ha + meandepth + pcturbancombined_hu12 + mean_secchi + Public.Beach + Public.Park, data = lakesDF, family = "binomial")
summary(logmod1a)

##try a linear model
linmod1a <- lm(VPY ~ mean_TP + lake_area_ha + meandepth + pcturbancombined_hu12 + mean_secchi + Public.Beach + Public.Park, data = lakesDF)
summary(linmod1a)

##try the poisson model
poismod1a <- glm(VPYRounded ~ mean_TP + lake_area_ha + meandepth + pcturbancombined_hu12 + mean_secchi + Public.Beach + Public.Park, data = lakesDF, family = "poisson")
summary(poismod1a)

##zip model  - this one is still weird: look into it more
zipmod1a <- zeroinfl(VPYRounded ~ mean_TP + lake_area_ha + meandepth + pcturbancombined_hu12 + mean_secchi + Public.Beach + Public.Park, data = lakesDF)
summary(zipmod1a)
```

New Lakes Data Set *stats for Lake Mendota*
===

```{r}
lakesDataSet <- read.csv("lakesDataSet.csv", header = T) ##this is just lake mendota
dim(lakesDataSet)
summary(lakesDataSet)

##get only the years that we want
lakesDataSet <- lakesDataSet%>%filter(year4 %in% c(2013, 2014, 2015, 2016, 2017))
dim(lakesDataSet)
summary(lakesDataSet)
lakesDataSet

##get monthly averages
lakesMonthly <- lakesDataSet%>%group_by(year4, month)%>%
  summarise(MeanAirTemp = mean(air_temp, na.rm = T), MeanRelHum = mean(rel_hum, na.rm = T), MeanWindSpeed = mean(wind_speed, na.rm = T), MeanWindDir = mean(wind_dir, na.rm = T), MeanChlor = mean(chlor, na.rm = T), MeanPhycocyanin = mean(phycocyanin, na.rm = T), MeanOptDoRaw = mean(opt_do_raw, na.rm = T), MeanOptDosatRaw = mean(opt_dosat_raw, na.rm = T), MeanOptWTemp = mean(opt_wtemp, na.rm = T), MeanPco2ppm = mean(pco2_ppm, na.rm = T), MeanPar = mean(par, na.rm = T))

dim(lakesMonthly)
summary(lakesMonthly)
lakesMonthly

lakesMonthly$year4 <- as.character(lakesMonthly$year4)
lakesMonthly$month <- as.character(lakesMonthly$month)

for (i in 1:nrow(lakesMonthly)) {
  if (nchar(lakesMonthly$month[i]) == 1) {
    lakesMonthly$month[i] <- paste("0", lakesMonthly$month[i], sep = "")
  }
}
```
