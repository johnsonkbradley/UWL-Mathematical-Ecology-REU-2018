...
myLakeDF
##write.csv(myLakeDF, "myLakes.csv", row.names = F)
...
Attach the visitors per year data to the myLakeDF
===

```{r}
myLakeDF <- myLakeDF %>% left_join(ourLakesSummary, by = c("lagosname1" = "Lake"))
myLakeDF
```

Take a look into some relationships between lake attributes now
===

```{r}
ggpairs(myLakeDF[-c(3,5),c(4,12,13,15,16,26,27,30,53)])
```
