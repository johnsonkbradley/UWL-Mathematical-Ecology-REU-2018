...
```{r}
mendota7.30.18 <- search_tweets(q = "Lake Mendota", n = 18000, include_rts = F, geocode = "43.104395,-89.415459,5mi")
ts_plot(mendota7.30.18)
mendota7.30.18$Name <- "Lake Mendota, WI"
...
```{r}
crater7.30.18 <- search_tweets(q = "Crater Lake", n = 18000, include_rts = F, geocode = "42.945247,-122.108234,10mi")
ts_plot(crater7.30.18)
crater7.30.18$Name <- "Crater Lake, OR"
...
```{r}
devils7.30.18 <- search_tweets(q = "Devil's Lake", n = 18000, include_rts = F, geocode = "43.415606,-89.730330,2mi")
ts_plot(devils7.30.18)
devils7.30.18$Name <- "Devil's Lake, WI"
...
```{r}
dodge7.30.18 <- search_tweets(q = "Governer Dodge", n = 18000, include_rts = F, geocode = "43.025734,-90.103683,2.5mi")
ts_plot(dodge7.30.18)
dodge7.30.18 ##no tweets for this time period
```

Lake Kegonsa State Park

```{r}
kegonsa7.30.18 <- search_tweets(q = "Lake Kegonsa", n = 18000, include_rts = F, geocode = "42.968061,-89.244261,2mi")
ts_plot(kegonsa7.30.18)
kegonsa7.30.18$Name <- "Lake Kegonsa State Park, WI"
...
```{r}
yellowstone7.30.18 <- search_tweets(q = "Yellowstone Lake", n = 18000, include_rts = F, geocode = "42.769699,-89.997597,2.5mi")
ts_plot(yellowstone7.30.18)
yellowstone7.30.18 ##no tweets for this time period
```

Big Foot Beach State Park

```{r}
bigfoot7.30.18 <- search_tweets(q = "Big Foot", n = 18000, include_rts = F, geocode = "42.568885,-88.432732,1mi")
ts_plot(bigfoot7.30.18)
bigfoot7.30.18 ##no tweets for this time period
```

Mirror Lake State Park

```{r}
mirror7.30.18 <- search_tweets(q = "Mirror Lake", n = 18000, include_rts = F, geocode = "43.566732,-89.816150,1.5mi")
ts_plot(mirror7.30.18)
mirror7.30.18$Name <- "Mirror Lake State Park, WI"
...
```{r}
peninsula7.30.18 <- search_tweets(q = "Peninsula", n = 18000, include_rts = F, geocode = "45.148148,-87.219086,2.5mi")
ts_plot(peninsula7.30.18)
peninsula7.30.18$Name <- "Peninsula State Park, WI"
...
```{r}
pattison7.30.18 <- search_tweets(q = "Pattison", n = 18000, include_rts = F, geocode = "46.529343,-92.120018,2mi")
ts_plot(pattison7.30.18)
pattison7.30.18 ##no tweets for this time period
```

Buckhorn State Park

```{r}
buckhorn7.30.18 <- search_tweets(q = "Buckhorn", n = 18000, include_rts = F, geocode = "43.934230,-89.995221,3mi")
ts_plot(buckhorn7.30.18)
buckhorn7.30.18$Name <- "Buckhorn State Park, WI"
...
```{r}
perrot7.30.18 <- search_tweets(q = "Perrot", n = 18000, include_rts = F, geocode = "44.024749,-91.490942,4.5mi")
ts_plot(perrot7.30.18)
perrot7.30.18 ##no tweets for this time period
```
...
##combining the two twitter data sets and filter unique
twitter30full <- rbind(twitter2, twitter30.a)
unique(twitter30full) ##same length so we're good
```
...
```{r}
ggplot(twitter30full, aes(x = DistanceDriven, y = Sentiment)) +
  theme_bw() +
...
devilsTwitterSimple <- devilsTwitter %>%
                  ...
                  mutate(TotalTweets = c(2,17,17,17,6,6,6,13,13,13,8,8,8), Percent = NumberTweets / TotalTweets)
...
##plot
ggplot(data = devilsTwitterLong%>%filter(!(Week == "2018-07-01")), aes(x = Week, y = value, fill = SentimentName)) +
